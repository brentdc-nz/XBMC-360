...
{
    return (projectm_eval_mem_buffer)calloc(PRJM_EVAL_MEM_BLOCKS, sizeof(PRJM_EVAL_F*));
}
...
PRJM_EVAL_F* prjm_eval_memory_allocate(projectm_eval_mem_buffer buffer, int index)
{
    ...
    if (index >= 0 && (block = index / PRJM_EVAL_MEM_ITEMSPERBLOCK) < PRJM_EVAL_MEM_BLOCKS)
    {
        ...
        if (!cur_block)
        {
            ...
            {
                cur_block = buffer[block] = (PRJM_EVAL_F *)calloc(sizeof(PRJM_EVAL_F), PRJM_EVAL_MEM_ITEMSPERBLOCK);
            }
            if (!cur_block)
            {
                index = 0;
