...
static int __pow[22], cluster, cmpcluster;
...
init (int *base)
{
  ...
  for (i = 1; i <= 21; i++)
    __pow[i] = __pow[i - 1] * 2;
  for (n = 21; __pow[n] > N || N % __pow[n]; n--)
    ;
  digits = (n + 3) / 4;
  for (i = 6; i; i--)
    if (__pow[i] * (tab_width + 1) < 80)
      break;
  per_row = __pow[i];
}
...
bt (int node_size)
{
  ...
  for (i = 1 - t[lev] % 2; i <= nn + (t[lev] >> nn) % 2; i++)
    {
      nn -= (p[lev] = i);
      clusters[lev] = cluster = (i && nn >= 0) ? __pow[i] : t[lev];
  ...
  for (ii = 0; ii < k; ii++)
    {
      ...
      fprintf (f, "\n\n#define %sLevel%d_%0*X 0x%0X\n", table_name,
	       best_lev - lev - 1, digits, x[i] * __pow[n - nn], ofs);
      kk = x[i] * cluster;
      if (!lev)
	if (name)
	  for (j = 0; j < cluster; j++)
	...
	for (j = 0; j < cluster; j++, kk++)
	  fprintf (f, "\n  %sLevel%d_%0*X,  /* %0*X..%0*X */", table_name,
		   best_lev - lev, digits,
		   tab[lev][kk] * __pow[n - nn - best_p[lev]], digits,
		   x[i] * __pow[n - nn] + j * __pow[n - nn - best_p[lev]], digits,
		   x[i] * __pow[n - nn] + (j + 1) * __pow[n - nn - best_p[lev]] -
		   1);
      ofs += cluster;
      jj = i;
      for (j = 0; j < k; j++)
...
write_source (void)
{
  ...
  for (i = best_lev - 1; i >= 0; i--)
	...
	fprintf (f, "/%d", j);
      if (i)
	fprintf (f, "%%%d +", __pow[best_p[best_lev - 1 - i]]);
