...
#else
				CGUIStaticItemPtr item = std::static_pointer_cast<CGUIStaticItem>(m_items[selected]);
#endif
...
void CGUIBaseContainer::UpdateVisibility(const CGUIListItem *item)
{
	...
	if (m_staticContent)
	{
		...
		{
			CGUIStaticItemPtr item = std::static_pointer_cast<CGUIStaticItem>(m_staticItems[i]);
