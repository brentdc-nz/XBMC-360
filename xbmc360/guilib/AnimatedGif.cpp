...
// Xbox 360 is big-endian (PowerPC) - GIF files store multi-byte
// values in little-endian format, so byte swapping is required.
#define SWAP16(X)    X = _byteswap_ushort(X)
#define SWAP32(X)    X = _byteswap_ulong(X)
