...
void CGUIWindowFileManager::UpdateItemCounts()
{
	for (int i = 0; i < 2; i++)
	{
		...
		CStdString items;
		if (selectedCount > 0)
			items.Format("%i/%i %s (%s)", selectedCount, totalCount, g_localizeStrings.Get(127).c_str(), CStringUtils::SizeToString(selectedSize).c_str());
		else
...
void CGUIWindowFileManager::OnStart(CFileItem *pItem)
{
	...
	if (pItem->IsPlayList())
	{
		...
		// TODO: ProcessAndStartPlaylist not yet available
	...
	if (pItem->IsXEX())
	{
		CXEXUtils::RunXex(pItem->GetPath().c_str());
		return ;
	}
	if (pItem->IsPicture())
	{
		// TODO: SlideShow not yet ported
		CLog::Log(LOGINFO, "FileManager: Picture viewing not yet implemented for: %s", pItem->GetPath().c_str());
	}
}
...
bool CGUIWindowFileManager::HaveDiscOrConnection( CStdString& strPath, int iDriveType )
{
	...
	{
		// TODO: DVD media detection
		CLog::Log(LOGWARNING, "FileManager: DVD media detection not implemented");
		return true;
	}
...
bool CGUIWindowFileManager::DoProcess(int iAction, CFileItemList & items, const CStdString& strDestFile)
{
	for (int iItem = 0; iItem < items.Size(); ++iItem)
	{
		...
		if (pItem->IsSelected())
		{
			...
			if ( URIUtils::IsRemote(pItem->GetPath()) )
	...
	if (CGUIDialogKeyboard::ShowAndGetInput(strFileName, g_localizeStrings.Get(16013), false))
	{
		...
		{ // special case for multipath renames - rename all the paths.
			...
			for (unsigned int i = 0; i < paths.size(); ++i)
			{
				...
				CStdString dirPath;
				URIUtils::GetDirectory(filePath, dirPath);
				URIUtils::AddFileToFolder(dirPath, strFileName, filePath);
...
void CGUIWindowFileManager::OnPopupMenu(int list, int item, bool bContextDriven /* = true */)
{
	...
	if (m_Directory[list]->IsVirtualDirectoryRoot())
	{
		...
		// TODO: SourcesMenu not available in xbmc360 yet
		...
		for (int i=0; i<m_vecItems[list]->Size(); ++i)
		{
			...
			if (pItem2->m_bIsFolder && pItem2->IsSelected())
			{
				...
				if (folderSize >= 0)
				{
					...
					if (folderSize == 0)
						pItem2->SetLabel2(CStringUtils::SizeToString(folderSize));
					else
...
bool CGUIWindowFileManager::CopyItem(const CFileItem *pItem, const CStdString& strDirectory, bool bSilent, CGUIDialogProgress* pProgress)
{
	...
	// prompt user for confirmation of file/folder copy
	if (!bSilent)
			if (!CGUIDialogYesNo::ShowAndGetInput(122, 125, 0, 0))
			return false;

	// Create a temporary item list containing the file/folder for copy
	CFileItemList items;
...
void CGUIWindowFileManager::SetInitialPath(const CStdString &path)
{
	...
	{
		m_Directory[0]->SetPath(strDestination = ""); // default to root
		CLog::Log(LOGINFO, "Attempting to default to: root");
	}
...
bool CGUIWindowFileManager::MoveItem(const CFileItem *pItem, const CStdString& strDirectory, bool bSilent, CGUIDialogProgress* pProgress)
{
	...
	// Create a temporary item list containing the file/folder for moving
	CFileItemList items;
