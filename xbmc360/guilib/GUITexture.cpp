...
bool bDick = true;
  //  color = g_graphicsContext.MergeAlpha(color); //FIXME
...
//        m_texture = g_TextureManager.GetTexture(m_info.filename); //TODO
      }
    }
 ...
 */
  ...
  {
    int images = g_TextureManager.Load(m_info.filename); //TODO
  ...
  if (!m_info.diffuse.IsEmpty())
  {
   // g_TextureManager.Load(m_info.diffuse); //TODO
   // m_diffuse = g_TextureManager.GetTexture(m_info.diffuse); //TODO
  }
...
void CGUITextureBase::FreeResources(bool immediately /* = false */) //TODO
{
...
void CGUITextureBase::UpdateAnimFrame() //TODO
{
  m_frameCounter++;
  unsigned int delay = 0;//m_texture.m_delays[m_currentFrame];//TODO
  if (!delay) delay = 100;
