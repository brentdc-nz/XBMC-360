...
void CGUIVisualisationControl::LoadVisualisation()
{
	...
	m_pVisualisation = factory.LoadVisualisation(m_currentVis);
...
//		g_graphicsContext.ApplyStateBlock();
		...
		g_graphicsContext.TLock();
		m_pVisualisation->Create(g_graphicsContext.Get3DDevice(),(int)(x+0.5f), (int)(y+0.5f), (int)(w+0.5f), (int)(h+0.5f));
		g_graphicsContext.TUnlock();
...
void CGUIVisualisationControl::Render()
{
	...
	if (m_pVisualisation)
	{
		if (m_bInitialized) // Waiting for the audio callback
		{
...
void CGUIVisualisationControl::CreateBuffers() //BRENT
{
	CSingleLock lock (m_critSection);
	ClearBuffers();
/*
