...
extern "C"
{
  ...
  {
	  CLog::Log(LOGERROR, "Socket Emu - gethostbyname() not implemented.");
...
//            address->sin_family,
 //           ntohs(address->sin_port),
  ...
  int __stdcall sckemu_ioctlsocket(int s, long cmd, DWORD FAR * argp)
  {
    ...
    {
      CLog::Log(LOGERROR, "sckemu_ioctlsocket invalid index:%i", s);
      return SOCKET_ERROR;
    }
  ...
  int __stdcall sckemu_recv(int s, char FAR * buf, int len, int flags)
  {
    if (s < 3 || s >= MAX_SOCKETS)
    {
      CLog::Log(LOGERROR, "sckemu_recv invalid index:%i", s);
      return SOCKET_ERROR;
    }
    SSocketData& socket = m_sockets[s];
    int len2, len3;
...
//    sprintf(_inetaddress, "%d.%d.%d.%d", in.S_un.S_un_b.s_b1, in.S_un.S_un_b.s_b2, in.S_un.S_un_b.s_b3, in.S_un.S_un_b.s_b4);
  ...
  {
    CLog::Log(LOGWARNING, "Untested function sckemu_gethostbyaddr called!");
    XNADDR xna;
    DWORD dwState;
