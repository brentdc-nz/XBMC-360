...
bool CAudioDecoder::Create(const CFileItem &file, __int64 seekOffset, unsigned int nBufferSize)
{
	...
	m_pcmBuffer.Create((int)std::max<unsigned int>(2, nBufferSize) * INTERNAL_BUFFER_LENGTH);

	// Reset our playback timing variables
	m_eof = false;

	// Get correct cache size
	unsigned int filecache = 500;//g_guiSettings.GetInt("cacheaudio.internet"); // TODO BRENT
...
{
	if (/*g_guiSettings.m_replayGain.iType != REPLAY_GAIN_NONE*/0) // TODO BRENT
	{
...
int CAudioDecoder::ReadPCMSamples(float *buffer, int numsamples, int *actualsamples)
{
	...
	switch (m_codec->m_BitsPerSample)
	{
		...
		*actualsamples /= 3;
		for (i = 0; i < *actualsamples; i++)
		{
			// Reconstruct 24-bit signed value from 3 bytes stored in native byte order
#ifdef _M_PPCBE
			// Big-endian: bytes are [MSB][mid][LSB]
			m_inputBuffer[i] = 1.0f / 0x7fffff * ((((int)((char *)m_pcmInputBuffer)[3*i]) << 16) | ((int)m_pcmInputBuffer[3*i+1] << 8) | ((int)m_pcmInputBuffer[3*i+2] << 0));
#else
...
float CAudioDecoder::GetReplayGain()  // TODO BRENT
{
...
/*	float replaydB = (float)g_guiSettings.m_replayGain.iNoGainPreAmp;
	...
	*/
	return 0;//replaygain; // TODO BRENT
}