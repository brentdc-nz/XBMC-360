#include "utils\Log.h"
#include "FLACcodec.h"
//#include "music\tags\FlacTag.h" // TODO BRENT

//using namespace MUSIC_INFO; // TODO BRENT
...
{
#if 0 // ifndef _HARDLINKED
	if (!m_dll.Load())
		return false;
#else
	m_libFlac = new CLibFlac(); // TODO CLEAN UP
#endif
	
	if (!m_file.Open(strFile/*, READ_CACHED*/)) // TODO BRENT
		return false;

	m_pFlacDecoder = m_libFlac->stream_decoder_new();
	...
	if (m_libFlac->stream_decoder_init_stream(m_pFlacDecoder, DecoderReadCallback,
                                                             DecoderSeekCallback,
                                                             DecoderTellCallback,
	...
	if (!m_libFlac->stream_decoder_process_until_end_of_metadata(m_pFlacDecoder))
	{
...
//	CFlacTag tag; //TODO BRENT
	
//	if (tag.Read(strFile))
//		m_replayGain = tag.GetReplayGain(); //TODO BRENT
...
__int64 FLACCodec::Seek(__int64 iSeekTime)
{
	...
	if(!m_libFlac->stream_decoder_seek_absolute(m_pFlacDecoder, (__int64)(iSeekTime*m_SampleRate)/1000))
		CLog::Log(LOGERROR, "FLACCodec::Seek - failed to seek");

	if(m_libFlac->stream_decoder_get_state(m_pFlacDecoder)==FLAC__STREAM_DECODER_SEEK_ERROR)
	{
		CLog::Log(LOGINFO, "FLACCodec::Seek - must reset decoder after seek");

		if(!m_libFlac->stream_decoder_flush(m_pFlacDecoder))
...
int FLACCodec::ReadPCM(BYTE *pBuffer, int size, int *actualsize)
{
	*actualsize=0;
	...
	if (m_libFlac->stream_decoder_get_state(m_pFlacDecoder) == FLAC__STREAM_DECODER_END_OF_STREAM)
		eof=true;
	...
	if (!eof)
	{
		...
		while(m_BufferSize < m_MaxFrameSize*4 &&
			m_libFlac->stream_decoder_get_state(m_pFlacDecoder)!=FLAC__STREAM_DECODER_END_OF_STREAM)
		{
			if (!m_libFlac->stream_decoder_process_single(m_pFlacDecoder))
			{
...
void FLACCodec::FreeDecoder()
{
	if (m_pFlacDecoder)
	{
		m_libFlac->stream_decoder_finish(m_pFlacDecoder);
		m_libFlac->stream_decoder_delete(m_pFlacDecoder);
		m_pFlacDecoder=NULL;
	}
}
...
FLAC__StreamDecoderWriteStatus FLACCodec::DecoderWriteCallback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
	...
	for(current_sample = 0; current_sample < frame->header.blocksize; current_sample++)
	{
		for(unsigned int channel = 0; channel < frame->header.channels; channel++)
		{
			...
			// On big-endian (Xbox 360), the significant bytes of a 32-bit int
			// are at the end of the memory layout. We need to offset the source
			// pointer so memcpy copies the correct (low) bytes, storing them
			// in native byte order for AudioDecoder::ReadPCMSamples to read back.
#ifdef _M_PPCBE
			memcpy(outptr, (BYTE*)&sample + (sizeof(int) - bytes_per_sample), bytes_per_sample);
#else
