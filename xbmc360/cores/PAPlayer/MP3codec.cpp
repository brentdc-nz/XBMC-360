...
int MP3Codec::Read(int size, bool init)
{
	...
	if ( m_InputBufferPos || m_CallAgainWithSameBuffer || (m_eof && m_Decoding) )
	{
		...
		if (size)
		{
			...
			if (init)
			{
				// TODO BRENT - port CMusicInfoTagLoaderMP3::IsID3v2Header
...
{
	return true; // Statically linked, always available
}

bool MP3Codec::SkipNext()
{
	return false; // TODO BRENT - CFile::SkipNext not available yet
