...
//#include "DVDInputStreams/DVDInputStreamNavigator.h" //TODO - Not yet ported
//#include "DVDInputStreams/DVDInputStreamTV.h" //TODO - Not yet ported

#include "DVDDemuxers/DVDDemux.h"
#include "DVDDemuxers/DVDDemuxUtils.h"
//#include "DVDDemuxers/DVDDemuxVobsub.h" //TODO - Not yet ported
#include "DVDDemuxers/DVDFactoryDemuxer.h"

#include "DVDCodecs/DVDCodecs.h"

//#include "DVDFileInfo.h" //TODO - Not yet ported

//#include "utils/LangCodeExpander.h" //TODO - Not yet ported
//#include "Util.h"
...
//#include "FileSystem/File.h" //TODO
#include "Settings.h"
...
static bool PredicateAudioPriority(const SelectionStream& lh, const SelectionStream& rh)
{
	...
	//PREDICATE_RETURN(StreamUtils::GetCodecPriority(lh.codec) //TODO - StreamUtils not yet ported
...
{
	if(/*!g_settings.m_currentVideoSettings.m_SubtitleOn*/false) 
	{
...
int CSelectionStreams::IndexOf(StreamType type, CDVDPlayer& p) const
{
	if (p.m_pInputStream && p.m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
	{
		...
		if(type == STREAM_AUDIO)
			id = -1; //TODO: ((CDVDInputStreamNavigator*)p.m_pInputStream)->GetActiveAudioStream();
		else if(type == STREAM_VIDEO)
			id = p.m_CurrentVideo.id;
		else if(type == STREAM_SUBTITLE)
			id = -1; //TODO: ((CDVDInputStreamNavigator*)p.m_pInputStream)->GetActiveSubtitleStream();
...
void CSelectionStreams::Update(CDVDInputStream* input, CDVDDemux* demuxer)
{
	...
	{
		//TODO - CDVDInputStreamNavigator not yet ported
		/*
...
bool CDVDPlayer::OpenInputStream()
{
	...
	//TODO - PlayListM3U not yet ported
	/*
...
void CDVDPlayer::HandlePlaySpeed()
{
	...
	if(caching == CACHESTATE_FULL)
	{
		...
		if(GetCachingTimes(level, delay, offset))
		{
			if(level  < 0.0)
			{
				CLog::Log(LOGWARNING, "Cache full - Cache filled before reaching required amount for continous playback");
				//TODO - g_application.m_guiDialogKaiToast.QueueNotification("Cache full", "Cache filled before reaching required amount for continous playback");
...
void CDVDPlayer::HandleMessages()
{
	...
	while (m_messenger.Get(&pMsg, 0) == MSGQ_OK)
	{
		...
		try
		{
			...
			{
				//TODO - CDVDInputStreamTV not yet ported
				/*
	...
	if (!m_bStop)
	{
		...
		if(m_StateInput.cache_bytes >= 0)
		{
			strBuf.AppendFormat(" cache:%s %2.0f%%"
												 , CStringUtils::SizeToString(m_State.cache_bytes).c_str()
...
{
	SelectionStream& s = m_SelectionStreams.Get(STREAM_SUBTITLE, iStream);
	//TODO - LangCodeExpander not yet ported
...
int64_t CDVDPlayer::GetTime()
{
	...
	return (int64_t)(m_State.time + DVD_TIME_TO_MSEC(offset));
}
...
{
	CSingleLock lock(m_StateSection);
	return (int64_t)(m_State.time_total);
}
...
bool CDVDPlayer::OpenSubtitleStream(int iStream, int source)
{
	...
	if(STREAM_SOURCE_MASK(source) == STREAM_SOURCE_DEMUX_SUB)
	{
		...
		//TODO - CDVDDemuxVobsub not yet ported
		/*
		...
		case ACTION_SHOW_VIDEOMENU:   // start button
			{
				...
				//TODO - GUIUserMessages not yet ported
				/*
...
void CDVDPlayer::UpdatePlayState(double timeout)
{
	...
	if (m_Edl.HasCut())
	{
		state.time        = m_Edl.RemoveCutTime((__int64)(state.time));
		state.time_total  = m_Edl.RemoveCutTime((__int64)(state.time_total));
	}
