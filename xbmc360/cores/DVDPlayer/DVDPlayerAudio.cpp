...
void CDVDPlayerAudio::Process()
{
	...
	while (!m_bStop)
	{
		...
		{      
			...
			while (!m_bStop && (unsigned int)m_dvdAudio.GetDelay() > audioframe.duration ) Sleep(5); //FIXME Marty
...
{ 
	...
	//if (m_speed == DVD_PLAYSPEED_PAUSE) m_dvdAudio.Pause(); //Marty FIXME
