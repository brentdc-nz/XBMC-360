...
// The Xbox 360 is big-endian PowerPC, so swap the little-endian zip values
#ifdef _XBOX
#define SwapLE16(X) ((unsigned short)((((unsigned short)(X) & 0x00ffu) << 8) | (((unsigned short)(X) & 0xff00u) >> 8)))
#define SwapLE32(X) ((unsigned int)((((unsigned int)(X) & 0x000000ffu) << 24) | (((unsigned int)(X) & 0x0000ff00u) << 8) | (((unsigned int)(X) & 0x00ff0000u) >> 8) | (((unsigned int)(X) & 0xff000000u) >> 24)))
#endif
...
bool CZipManager::GetZipList(const CStdString& strPath, vector<SZipEntry>& items)
{
	...
	if( SwapLE32(hdr) != ZIP_LOCAL_HEADER )
	{
	...
	for (int nb=1; !found && (nb <= nbBlock); nb++)
	{
		...
		{
			if ( SwapLE32(*((unsigned int*)(buffer+i))) == ZIP_END_CENTRAL_HEADER )
			{
	...
	while (mFile.GetPosition() < cdirOffset + cdirSize)
	{
		...
		ze.elength = SwapLE16(ze.elength);
...
{
	info.header = SwapLE32(*(unsigned int*)buffer);
	info.version = SwapLE16(*(unsigned short*)(buffer+4));
	info.flags = SwapLE16(*(unsigned short*)(buffer+6));
	info.method = SwapLE16(*(unsigned short*)(buffer+8));
	info.mod_time = SwapLE16(*(unsigned short*)(buffer+10));
	info.mod_date = SwapLE16(*(unsigned short*)(buffer+12));
	info.crc32 = SwapLE32(*(unsigned int*)(buffer+14));
	info.csize = SwapLE32(*(unsigned int*)(buffer+18));
	info.usize = SwapLE32(*(unsigned int*)(buffer+22));
	info.flength = SwapLE16(*(unsigned short*)(buffer+26));
	info.elength = SwapLE16(*(unsigned short*)(buffer+28));
}
...
void CZipManager::readCHeader(const char* buffer, SZipEntry& info)
{
	...
	info.version = SwapLE16(*(unsigned short*)(buffer+6));
	info.flags = SwapLE16(*(unsigned short*)(buffer+8));
	info.method = SwapLE16(*(unsigned short*)(buffer+10));
	info.mod_time = SwapLE16(*(unsigned short*)(buffer+12));
	info.mod_date = SwapLE16(*(unsigned short*)(buffer+14));
	info.crc32 = SwapLE32(*(unsigned int*)(buffer+16));
	info.csize = SwapLE32(*(unsigned int*)(buffer+20));
	info.usize = SwapLE32(*(unsigned int*)(buffer+24));
	info.flength = SwapLE16(*(unsigned short*)(buffer+28));
	info.eclength = SwapLE16(*(unsigned short*)(buffer+30));
	info.clength = SwapLE16(*(unsigned short*)(buffer+32));
	// Skip disk number start, internal/external file attributes
	info.lhdrOffset = SwapLE32(*(unsigned int*)(buffer+42));
