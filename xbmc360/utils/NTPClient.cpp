...
public:
	...
	bool Create();
	bool Connect(CStdString strHostAddress, int nPort);
...
bool CNtpSocket::Connect(CStdString strHostAddress, int nPort)
{
...
//		return false;
	...
	XNetDnsLookup(strHostAddress.c_str(), hScktEvent, &pxndns);
	...
	debugSendSocketAddr.sin_addr.s_addr = inet_addr(strHostAddress.c_str());
	debugSendSocketAddr.sin_addr = pxndns->aina[0];
	debugSendSocketAddr.sin_port = 123;

	XNetDnsRelease(pxndns);

	return (connect(m_hSocket, (struct sockaddr* )&debugSendSocketAddr, sizeof(debugSendSocketAddr)) == 0);
}

bool CNtpSocket::Send(char* pszBuf, int nBuf)
{
...
int CNtpSocket::Receive(LPSTR pszBuf, int nBuf)
{
	...
	int iRslt = recv((SOCKET)m_hSocket, pszBuf, nBuf, 0);
...
void CNTPClient::SyncTime()
{
	...
	if(m_ThreadHandle)
		...
		CLog::Log(LOGERROR, "NTP: Failed To Create Thread");
}

bool CNTPClient::UpdateNeeded()
{
...
bool CNTPClient::DoTimeSync(CStdString strHostName, NtpServerResponse& response, int nPort)
{
	...
	if(!pSocket->Create())
	{
		CLog::Log(LOGERROR, "NTP: Failed to create client socket, GetLastError returns: %d\n", GetLastError());
		return false;
	}

	// Connect to the SNTP server
	if(!pSocket->Connect(strHostName, nPort))
	{
		CLog::Log(LOGERROR, "NTP: Could not connect to the SNTP server %s on port %d, GetLastError returns: %d\n", strHostName, nPort, GetLastError());
	...
	else
	{
		...
		if(!pSocket->Send((char*)&nbi, /*nSendSize*/48)) // FIXME: Why are we grtting 68?
		{
			CLog::Log(LOGERROR, "NTP: Failed to send NTP request to the SNTP server, GetLastError returns %d\n", GetLastError());
		...
		{
			CLog::Log(LOGERROR, "NTP: Unable to wait for reply from the SNTP server, GetLastError returns %d\n", WSAETIMEDOUT);
		...
		if(!pSocket->Receive((LPSTR)&nfp, /*nReceiveSize*/48)) // FIXME: Why are we getting 68 back?
		{
			CLog::Log(LOGERROR, "NTP: Unable to read reply from the SNTP server, GetLastError returns %d\n", GetLastError());
