...
CStdString CID3Tag::ToStringCharset(const id3_ucs4_t* ucs4, id3_field_textencoding encoding) const
{
	...
	{
		id3_latin1_t* latin1 = m_libID3Tag.id3_ucs4_latin1duplicate(ucs4);
		g_charsetConverter.unknownToUTF8((LPCSTR)latin1, strValue);
		m_libID3Tag.id3_latin1_free(latin1);
	}
	else
	{
		// convert to UTF-8
		id3_utf8_t* utf8 = m_libID3Tag.id3_ucs4_utf8duplicate(ucs4);
		strValue = (LPCSTR)utf8;
		m_libID3Tag.id3_utf8_free(utf8);
	}
...
id3_ucs4_t* CID3Tag::StringCharsetToUcs4(const CStdString& str) const
{
	...
	return m_libID3Tag.id3_utf8_ucs4duplicate((id3_utf8_t*)str.c_str());
}
...
bool CID3Tag::Read(const CStdString& strFile)
{
	...
	// Use CFile to read through XBMC VFS (handles SMB, etc.)
	// Read only the ID3 tag data, not the entire file.
	XFILE::CFile file;
	if (!file.Open(strFile))
		return false;

	__int64 fileLength = file.GetLength();
	if (fileLength <= 0)
	{
		file.Close();
		return false;
	...
	id3_byte_t header[ID3_TAG_QUERYSIZE];
	if (file.Read(header, ID3_TAG_QUERYSIZE) != ID3_TAG_QUERYSIZE)
	{
		file.Close();
		return false;
	}

	signed long tagSize = m_libID3Tag.id3_tag_query(header, ID3_TAG_QUERYSIZE);
	if (tagSize > 0)
	{
		// ID3v2 tag found - read the full tag data
		id3_byte_t* tagData = new id3_byte_t[(unsigned int)tagSize];
		file.Seek(0);
		unsigned int bytesRead = file.Read(tagData, (unsigned int)tagSize);
		if (bytesRead == (unsigned int)tagSize)
			m_tag = m_libID3Tag.id3_tag_parse(tagData, tagSize);
		delete[] tagData;
	}

	// If no ID3v2, check for ID3v1 at the end of file
	if (!m_tag && fileLength >= 128)
	{
		id3_byte_t tail[128];
		file.Seek(fileLength - 128);
		if (file.Read(tail, 128) == 128)
		{
			tagSize = m_libID3Tag.id3_tag_query(tail, 128);
			if (tagSize == 128)
				m_tag = m_libID3Tag.id3_tag_parse(tail, 128);
		}
	}
	...
	m_libID3Tag.id3_tag_delete(m_tag);
	m_tag = NULL;
	return true;
}
...
bool CID3Tag::Write(const CStdString& strFile)
{
	...
	id3_file* id3file = m_libID3Tag.id3_file_open(strFile.c_str(), ID3_FILE_MODE_READWRITE);
	if (!id3file)
		return false;

	m_tag = m_libID3Tag.id3_file_tag(id3file);
	if (!m_tag)
	{
		m_libID3Tag.id3_file_close(id3file);
		return false;
	}
	...
	m_libID3Tag.id3_tag_options(m_tag, ID3_TAG_OPTION_COMPRESSION, 0);
	m_libID3Tag.id3_tag_options(m_tag, ID3_TAG_OPTION_CRC, 0);
	m_libID3Tag.id3_tag_options(m_tag, ID3_TAG_OPTION_UNSYNCHRONISATION, 0);
	m_libID3Tag.id3_tag_options(m_tag, ID3_TAG_OPTION_ID3V1, 1);

	bool success = (m_libID3Tag.id3_file_update(id3file) != -1) ? true : false;
...
CStdString CID3Tag::GetArtist() const
{
	...
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getartist(m_tag, &encoding);
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getalbum(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getalbumartist(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_gettitle(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_gettrack(m_tag, &encoding);
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getpartofset(m_tag, &encoding);
	return atoi(ToStringCharset(ucs4, encoding));
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getyear(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	id3_ucs4_list_t* list = m_libID3Tag.id3_metadata_getgenres(m_tag, &encoding);
	CStdString genre;
	if (list)
	{
		...
		m_libID3Tag.id3_ucs4_list_free(list);
	}
	return genre;
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getcomment(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getencodedby(m_tag, &encoding);
	return ToStringCharset(ucs4, encoding);
}
...
CStdString CID3Tag::GetLyrics() const
{
	...
	frame = m_libID3Tag.id3_tag_findframe(m_tag, "USLT", 0);
	if (!frame) return "";

	/* Find the encoding used, stored in frame 0 */
	field = m_libID3Tag.id3_frame_field(frame, 0);

	if (field && (m_libID3Tag.id3_field_type(field) == ID3_FIELD_TYPE_TEXTENCODING))
		encoding = m_libID3Tag.id3_field_gettextencoding(field);

	/* The last field contains the data */
	field = m_libID3Tag.id3_frame_field(frame, frame->nfields - 1);
	if (!field) return "";

	if (field->type != ID3_FIELD_TYPE_STRINGFULL) return "";

	ucs4 = m_libID3Tag.id3_field_getfullstring(field);
...
{
	return m_libID3Tag.id3_metadata_getrating(m_tag);
}

bool CID3Tag::GetCompilation() const
{
	id3_field_textencoding encoding = ID3_FIELD_TEXTENCODING_ISO_8859_1;
	const id3_ucs4_t* ucs4 = m_libID3Tag.id3_metadata_getcompilation(m_tag, &encoding);
...
{
	return (m_libID3Tag.id3_metadata_haspicture(m_tag, pictype) > 0 ? true : false);
}

CStdString CID3Tag::GetPictureMimeType(id3_picture_type pictype) const
{
	return (LPCSTR)m_libID3Tag.id3_metadata_getpicturemimetype(m_tag, pictype);
}

const BYTE* CID3Tag::GetPictureData(id3_picture_type pictype, id3_length_t* length) const
{
	return m_libID3Tag.id3_metadata_getpicturedata(m_tag, pictype, length);
}

const BYTE* CID3Tag::GetUniqueFileIdentifier(const CStdString& strOwnerIdentifier, id3_length_t* length) const
{
	return m_libID3Tag.id3_metadata_getuniquefileidentifier(m_tag, strOwnerIdentifier.c_str(), length);
}

CStdString CID3Tag::GetUserText(const CStdString& strDescription) const
{
	return ToStringCharset(m_libID3Tag.id3_metadata_getusertext(m_tag, strDescription.c_str()), ID3_FIELD_TEXTENCODING_ISO_8859_1);
}

bool CID3Tag::GetFirstNonStandardPictype(id3_picture_type* pictype) const
{
	return (m_libID3Tag.id3_metadata_getfirstnonstandardpictype(m_tag, pictype) > 0 ? true : false);
}

void CID3Tag::SetArtist(const CStdString& strValue)
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setartist(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}

void CID3Tag::SetAlbum(const CStdString& strValue)
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setalbum(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setalbumartist(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_settitle(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
void CID3Tag::SetTrack(int n)
{
	...
	strValue.Format("%d", n);
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_settrack(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
void CID3Tag::SetPartOfSet(int n)
{
	...
	strValue.Format("%d", n);
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setpartofset(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setyear(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setgenre(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setencodedby(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
{
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setcomment(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}

void CID3Tag::SetRating(char rating)
{
	m_libID3Tag.id3_metadata_setrating(m_tag, rating);
}
...
void CID3Tag::SetCompilation(bool compilation)
{
	...
	id3_ucs4_t* ucs4 = StringCharsetToUcs4(strValue);
	m_libID3Tag.id3_metadata_setcompilation(m_tag, ucs4);
	m_libID3Tag.id3_ucs4_free(ucs4);
}
...
CStdString CID3Tag::ParseMP3Genre(const CStdString& str) const
{
	...
	while (!strTemp.IsEmpty())
	{
		...
		else
		{
			...
			if (CStringUtils::IsNaturalNumber(t))
			{
				id3_ucs4_t* ucs4 = m_libID3Tag.id3_latin1_ucs4duplicate((id3_latin1_t*)t.c_str());
				const id3_ucs4_t* genre = m_libID3Tag.id3_genre_name(ucs4);
				m_libID3Tag.id3_ucs4_free(ucs4);
