                        ...
                        NPT_Timeout        frequency /* = 50000 */)
{
    ...
    // Xbox 360 retail XNet will not auto-bind an unbound UDP socket on the
    // first sendto() — the packet goes out with an invalid source address
    // and responses never route back.  Explicitly bind to ANY:0 to get a
    // valid local (ip, ephemeral port) pair before sending.
    socket->Bind(NPT_SocketAddress(NPT_IpAddress::Any, 0), false);
