...
#if defined(_XBOX)
    // Xbox 360 XNet "patch" socket options - required on retail consoles to
    // allow raw UDP broadcast/multicast to/from unregistered peers. Without
    // these, sendto() returns OK but packets are filtered by XNet's secure
    // stack and inbound datagrams from non-secure peers are dropped.
    // Must be set BEFORE SO_BROADCAST.  Matches FreestyleDash's UdpClient
    // which is known to work on modded retail.
    setsockopt(m_SocketFdReference->m_SocketFd,
               SOL_SOCKET,
               0x5802,
               (SocketOption)&option,
...
NPT_BsdUdpMulticastSocket::JoinGroup(const NPT_IpAddress& group,
                                     const NPT_IpAddress& iface)
{
    // Xbox 360 XDK does not support IP_ADD_MEMBERSHIP via setsockopt
    return NPT_SUCCESS;
}
...
NPT_BsdUdpMulticastSocket::LeaveGroup(const NPT_IpAddress& group,
                                      const NPT_IpAddress& iface)
{
    // Xbox 360 XDK does not support IP_DROP_MEMBERSHIP via setsockopt
    return NPT_SUCCESS;
}
...
{
    // Xbox 360 XDK does not support IP_MULTICAST_IF via setsockopt
    return NPT_SUCCESS;
}
...
{
    // Xbox 360 XDK does not support IP_MULTICAST_TTL via setsockopt
    return NPT_SUCCESS;
}
...
#if defined(_XBOX)
    // Xbox 360 XNet "patch" socket options - required on retail consoles to
    // allow TCP connect() to unregistered peers (e.g. DLNA/UPnP servers).
    // Without these, the TCP handshake never completes because XNet-secure
    // drops SYN to peers that aren't part of an XLive secure session.
    // Same two options FreestyleDash uses for UDP broadcast.
    int option = 1;
    setsockopt(m_SocketFdReference->m_SocketFd,
               SOL_SOCKET,
               0x5802,
