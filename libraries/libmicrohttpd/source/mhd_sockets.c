...
#ifdef _XBOX
    /* Xbox 360 has no loopback interface; use the console's own IP */
    {
      XNADDR xnAddr;
      memset(&xnAddr, 0, sizeof(xnAddr));
      while (XNetGetTitleXnAddr(&xnAddr) == XNET_GET_XNADDR_PENDING) { Sleep(1); }
      listen_addr.sin_addr = xnAddr.ina;
    }
#else
