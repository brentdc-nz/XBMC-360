...
#ifdef _XBOX
    /* Xbox 360 XNet socket options required on retail consoles.
       Options 0x5802 and 0x5801 disable the secure network layer
       on this socket so that inbound connections from unsecured
       peers (i.e. any LAN client) are accepted. */
    {
      int xopt = 1;
      setsockopt (daemon->listen_fd, SOL_SOCKET, 0x5802,
                  (const char *) &xopt, sizeof (xopt));
      setsockopt (daemon->listen_fd, SOL_SOCKET, 0x5801,
                  (const char *) &xopt, sizeof (xopt));
    }
#endif
    if (MHD_D_IS_USING_SELECT_ (daemon) &&
