/*
 * VC-1 and WMV3 decoder - DSP functions AltiVec/VMX-optimized for Xbox 360
...
/**
 * @file
 * Xbox 360 VMX-optimized VC-1 inverse transforms for FFmpeg 1.2
 *
 * Changes from 0.6:
 *   - DCTELEM replaced with int16_t
 *   - Registration target changed from DSPContext to VC1DSPContext
 *   - Init function renamed to ff_vc1dsp_init_ppc(VC1DSPContext*)
 */
...
/**
 * Simple GCC AltiVec to MSVC VMX intrinsic translation macros
 */
...
do { \
    ...
    A1 = vec_mergeh_s16(a, e);  B1 = vec_mergel_s16(a, e); \
    C1 = vec_mergeh_s16(b, f);  D1 = vec_mergel_s16(b, f); \
    E1 = vec_mergeh_s16(c, g);  F1 = vec_mergel_s16(c, g); \
    G1 = vec_mergeh_s16(d, h);  H1 = vec_mergel_s16(d, h); \
    A2 = vec_mergeh_s16(A1, E1);  B2 = vec_mergel_s16(A1, E1); \
    C2 = vec_mergeh_s16(B1, F1);  D2 = vec_mergel_s16(B1, F1); \
    E2 = vec_mergeh_s16(C1, G1);  F2 = vec_mergel_s16(C1, G1); \
    G2 = vec_mergeh_s16(D1, H1);  H2 = vec_mergel_s16(D1, H1); \
    a = vec_mergeh_s16(A2, E2);  b = vec_mergel_s16(A2, E2); \
    c = vec_mergeh_s16(B2, F2);  d = vec_mergel_s16(B2, F2); \
    e = vec_mergeh_s16(C2, G2);  f = vec_mergel_s16(C2, G2); \
    g = vec_mergeh_s16(D2, H2);  h = vec_mergel_s16(D2, H2); \
} while (0)

/* Main steps of 8x8 VC-1 inverse transform */
...
do { \
    ...
    s0 = vec_add_s32(t4, t0);  s1 = vec_add_s32(t5, t1); \
    s2 = vec_add_s32(t6, t2);  s3 = vec_add_s32(t7, t3); \
    s4 = vec_sub_s32(t7, t3);  s5 = vec_sub_s32(t6, t2); \
    s6 = vec_sub_s32(t5, t1);  s7 = vec_sub_s32(t4, t0); \
} while(0)

#define SHIFT_HOR8(s0, s1, s2, s3, s4, s5, s6, s7) \
do { \
    s0 = vec_sra_s32(s0, vec_3);  s1 = vec_sra_s32(s1, vec_3); \
    s2 = vec_sra_s32(s2, vec_3);  s3 = vec_sra_s32(s3, vec_3); \
    s4 = vec_sra_s32(s4, vec_3);  s5 = vec_sra_s32(s5, vec_3); \
    s6 = vec_sra_s32(s6, vec_3);  s7 = vec_sra_s32(s7, vec_3); \
} while(0)

#define SHIFT_VERT8(s0, s1, s2, s3, s4, s5, s6, s7) \
do { \
    s0 = vec_sra_s32(s0, vec_7);  s1 = vec_sra_s32(s1, vec_7); \
    s2 = vec_sra_s32(s2, vec_7);  s3 = vec_sra_s32(s3, vec_7); \
 ...
 */
static void vc1_inv_trans_8x8_vmx(int16_t block[64])
{
    __vector4 src0, src1, src2, src3, src4, src5, src6, src7;
    __vector4 s0, s1, s2, s3, s4, s5, s6, s7;
    __vector4 s8, s9, sA, sB, sC, sD, sE, sF;
    __vector4 t0, t1, t2, t3, t4, t5, t6, t7;

    __vector4 vec_64 = vec_sl_s32(vec_splat_s32(4), vec_splat_u32(4));
    __vector4 vec_7  = vec_splat_u32(7);
    __vector4 vec_4  = vec_splat_u32(4);
    __vector4 vec_4s = vec_splat_s32(4);
    __vector4 vec_3  = vec_splat_u32(3);
    __vector4 vec_2  = vec_splat_u32(2);
    __vector4 vec_1s = vec_splat_s32(1);
    __vector4 vec_1  = vec_splat_u32(1);
    ...
    s0 = vec_unpackl_s16(src0);  s8 = vec_unpackh_s16(src0);
    s1 = vec_unpackl_s16(src1);  s9 = vec_unpackh_s16(src1);
    s2 = vec_unpackl_s16(src2);  sA = vec_unpackh_s16(src2);
    s3 = vec_unpackl_s16(src3);  sB = vec_unpackh_s16(src3);
    s4 = vec_unpackl_s16(src4);  sC = vec_unpackh_s16(src4);
    s5 = vec_unpackl_s16(src5);  sD = vec_unpackh_s16(src5);
    s6 = vec_unpackl_s16(src6);  sE = vec_unpackh_s16(src6);
    s7 = vec_unpackl_s16(src7);  sF = vec_unpackh_s16(src7);
    ...
    src0 = vec_pack_s32(s8, s0);  src1 = vec_pack_s32(s9, s1);
    src2 = vec_pack_s32(sA, s2);  src3 = vec_pack_s32(sB, s3);
    src4 = vec_pack_s32(sC, s4);  src5 = vec_pack_s32(sD, s5);
    src6 = vec_pack_s32(sE, s6);  src7 = vec_pack_s32(sF, s7);
    ...
    vec_st(src0,   0, block);  vec_st(src1,  16, block);
    vec_st(src2,  32, block);  vec_st(src3,  48, block);
    vec_st(src4,  64, block);  vec_st(src5,  80, block);
    vec_st(src6,  96, block);  vec_st(src7, 112, block);
}

/**
 * Initialize VC-1 DSP functions for Xbox 360.
 * FFmpeg 1.2 uses VC1DSPContext instead of DSPContext.
 */
av_cold void ff_vc1dsp_init_ppc(VC1DSPContext *dsp)
{
    dsp->vc1_inv_trans_8x8 = vc1_inv_trans_8x8_vmx;
    /* dsp->vc1_inv_trans_8x4 can be added once 8x4 VMX code is verified */
}
