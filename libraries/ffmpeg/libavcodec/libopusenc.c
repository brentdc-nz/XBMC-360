...
static const AVOption libopus_options[] = {
    { "application",    "Intended application type",           OFFSET(application),    AV_OPT_TYPE_INT,   {OPUS_APPLICATION_AUDIO }, OPUS_APPLICATION_VOIP, OPUS_APPLICATION_RESTRICTED_LOWDELAY, FLAGS, "application" },
        { "voip",           "Favor improved speech intelligibility",   0, AV_OPT_TYPE_CONST, {OPUS_APPLICATION_VOIP },                0, 0, FLAGS, "application" },
        { "audio",          "Favor faithfulness to the input",         0, AV_OPT_TYPE_CONST, {OPUS_APPLICATION_AUDIO },               0, 0, FLAGS, "application" },
        { "lowdelay",       "Restrict to only the lowest delay modes", 0, AV_OPT_TYPE_CONST, {OPUS_APPLICATION_RESTRICTED_LOWDELAY }, 0, 0, FLAGS, "application" },
    { "frame_duration", "Duration of a frame in milliseconds", OFFSET(frame_duration), AV_OPT_TYPE_FLOAT, { 0 }, 2.5, 60.0, FLAGS },
    { "packet_loss",    "Expected packet loss percentage",     OFFSET(packet_loss),    AV_OPT_TYPE_INT,   {0 },    0,   100,  FLAGS },
    { "vbr",            "Variable bit rate mode",              OFFSET(vbr),            AV_OPT_TYPE_INT,   {1 },    0,   2,    FLAGS, "vbr" },
        { "off",            "Use constant bit rate", 0, AV_OPT_TYPE_CONST, {0 }, 0, 0, FLAGS, "vbr" },
        { "on",             "Use variable bit rate", 0, AV_OPT_TYPE_CONST, {1 }, 0, 0, FLAGS, "vbr" },
        { "constrained",    "Use constrained VBR",   0, AV_OPT_TYPE_CONST, {2 }, 0, 0, FLAGS, "vbr" },
    { NULL },
};
...
static const enum AVSampleFormat _ff_libopusenc_fmts_75[] = { AV_SAMPLE_FMT_S16,
                                                      AV_SAMPLE_FMT_FLT,
                                                      AV_SAMPLE_FMT_NONE };
AVCodec ff_libopus_encoder = {
        "libopus", /* name */
        NULL_IF_CONFIG_SMALL("libopus Opus"), /* long_name */
        AVMEDIA_TYPE_AUDIO, /* type */
        AV_CODEC_ID_OPUS, /* id */
        CODEC_CAP_DELAY | CODEC_CAP_SMALL_LAST_FRAME, /* capabilities */
        0, /* supported_framerates */
        0, /* pix_fmts */
        libopus_sample_rates, /* supported_samplerates */
        _ff_libopusenc_fmts_75, /* sample_fmts */
        ff_vorbis_channel_layouts, /* channel_layouts */
        0, /* max_lowres */
        &libopus_class, /* priv_class */
        0, /* profiles */
        sizeof(LibopusEncContext), /* priv_data_size */
        0, /* next */
        0, /* init_thread_copy */
        0, /* update_thread_context */
        libopus_defaults, /* defaults */
        0, /* init_static_data */
        libopus_encode_init, /* init */
        0, /* encode_sub */
        libopus_encode, /* encode2 */
        0, /* decode */
        libopus_encode_close, /* close */
    };
