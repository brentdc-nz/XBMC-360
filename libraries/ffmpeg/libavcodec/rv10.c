...
static int rv20_decode_picture_header(RVDecContext *rv)
{
    ...
    if(rpr_bits){
        ...
        if(new_w != s->width || new_h != s->height){
            ...
            if (!old_aspect.num)
                old_aspect = av_make_q(1, 1);
            if (2 * new_w * s->height == new_h * s->width)
                s->avctx->sample_aspect_ratio = av_mul_q(old_aspect, av_make_q(2, 1));
            if (new_w * s->height == 2 * new_h * s->width)
                s->avctx->sample_aspect_ratio = av_mul_q(old_aspect, av_make_q(1, 2));
...
AVCodec ff_rv10_decoder = {
        ...
        0, /* priv_class */
        0, /* profiles */
        sizeof(RVDecContext), /* priv_data_size */
        0, /* next */
        0, /* init_thread_copy */
        0, /* update_thread_context */
        0, /* defaults */
        0, /* init_static_data */
        rv10_decode_init, /* init */
        0, /* encode_sub */
        0, /* encode2 */
        rv10_decode_frame, /* decode */
        rv10_decode_end, /* close */
    };
