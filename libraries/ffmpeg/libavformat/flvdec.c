...
static int flv_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 for(;;avio_skip(s->pb, 4)){ /* pkt size is repeated at end. skip it */
    ...
    if(i == s->nb_streams){
        ...
        { static int _cla_flvdec_1[] = {AVMEDIA_TYPE_VIDEO, AVMEDIA_TYPE_AUDIO, AVMEDIA_TYPE_DATA};
        st = create_stream(s, _cla_flvdec_1[stream_type]); }
        if (!st)
            return AVERROR(ENOMEM);
...
AVInputFormat ff_flv_demuxer = {
    ...
    flv_probe, /* read_probe */
    flv_read_header, /* read_header */
    flv_read_packet, /* read_packet */
    flv_read_close, /* read_close */
    flv_read_seek, /* read_seek */
};
