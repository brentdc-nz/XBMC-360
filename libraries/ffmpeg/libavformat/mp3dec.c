...
static int mp3_parse_vbr_tags(AVFormatContext *s, AVStream *st, int64_t base)
{
    ...
    if (v == MKBETAG('X', 'i', 'n', 'g') || is_cbr) {
        ...
        if (v & XING_FLAG_TOC && frames)
            read_xing_toc(s, size, av_rescale_q(frames, av_make_q(spf, c.sample_rate),
                                    st->time_base));
        if(v & 8)
            avio_skip(s->pb, 4);
    ...
    if(frames)
        st->duration = av_rescale_q(frames, av_make_q(spf, c.sample_rate),
                                    st->time_base);
...
AVInputFormat ff_mp3_demuxer = {
    "mp3", /* name */
    NULL_IF_CONFIG_SMALL("MP2/3 (MPEG audio layer 2/3)"), /* long_name */
    AVFMT_GENERIC_INDEX, /* flags */
    "mp2,mp3,m2a", /* extensions */
    0, /* codec_tag */
    0, /* priv_class */
    0, /* next */
    0, /* raw_codec_id */
    sizeof(MP3Context), /* priv_data_size */
    mp3_read_probe, /* read_probe */
    mp3_read_header, /* read_header */
    mp3_read_packet, /* read_packet */
    0, /* read_close */
    mp3_seek, /* read_seek */
};
