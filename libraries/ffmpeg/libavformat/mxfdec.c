...
static int mxf_read_timecode_component(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x1501:
        ...
        break;
    case 0x1502:
        mxf_timecode->rate = av_make_q(avio_rb16(pb), 1);
        break;
    case 0x1503:
...
static int mxf_parse_structural_metadata(MXFContext *mxf)
{
    ...
    for (i = 0; i < material_package->tracks_count; i++) {
        ...
        if (material_track->edit_rate.num <= 0 || material_track->edit_rate.den <= 0) {
            ...
            material_track->edit_rate = av_make_q(25, 1);
        }
...
static int mxf_compute_sample_count(MXFContext *mxf, int stream_index, uint64_t *sample_count)
{
    ...
    if (!spf) {
        ...
        *sample_count = av_q2d(av_mul_q(av_make_q(mxf->current_edit_unit, 1),
...
AVInputFormat ff_mxf_demuxer = {
    ...
    mxf_probe, /* read_probe */
    mxf_read_header, /* read_header */
    mxf_read_packet, /* read_packet */
    mxf_read_close, /* read_close */
    mxf_read_seek, /* read_seek */
};
